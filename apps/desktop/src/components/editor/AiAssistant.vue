...
import { aiStream, aiCancelStream, saveAiConversation, saveAiRun, saveAiRunState, loadAiConversations, loadAiRuns, deleteAiConversation, listSchemas, listTables, type AiConversation, type AiRun, type AiRunStatus } from "@/lib/backend/api";
...
type PendingAutoSend = { conversationId: string; text: string; messages: ChatMessage[]; mode: AiAssistantMode; action: AiAction; pluginContext?: AiPluginContext };
...
const activePlaceholder = computed(() => (pluginContext.value ? t("ai.pluginFollowUp") : `${t(`ai.placeholders.${activeAction.value}`)} ${t("ai.tableMentionPlaceholderHint")}`));
...
async function loadMentionCandidates(query: string) {
  if (pluginContext.value || !props.connection || !props.tab?.connectionId || !props.tab.database) return;
...
async function send() {
  ...
  const requestedAction = runPluginContext ? "general" : auto ? auto.action : activeAction.value;
  const requestedMode = runPluginContext ? "ask" : auto ? auto.mode : assistantMode.value;
  ...
  try {
    ...
    if (detachedRun || aiGenerationGuard.isCurrent(myGeneration)) {
      ...
      if (msg && !msg.agentSteps?.length && connection && tab) {
...
function buildConversationSnapshot(targetConversationId: string, targetMessages: ChatMessage[], connectionName: string, database: string, createdAt = new Date().toISOString()): AiConversation | null {
  ...
  return {
    ...
    title: first?.pluginContext?.title || (first ? messageTitle(first).slice(0, 50) : "Untitled"),
...
defineExpose({ openPluginConversation, triggerAction, setPrompt, addTableMention, clearContextReferences, selectConversationById });
