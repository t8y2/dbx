...
async function resolveSqlHoverTooltip(currentView: EditorViewType, pos: number) {
  ...
  try {
    ...
    if (!table && !usesLocalOnlyCompletionMetadata()) {
      const remoteHoverTables = await connectionStore.listCompletionTables(props.connectionId, props.database, tableLookupName, MAX_COMPLETION_TABLES, semanticTarget?.schema ?? props.schema, false, props.schema, props.catalog);
