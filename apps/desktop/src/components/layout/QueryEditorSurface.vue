...
defineExpose<QueryEditorSurfaceHandle>({
  ...
  insertRedisCommand: (command: string, connectionId?: string) => contentAreaRef.value?.insertRedisCommand(command, connectionId) ?? Promise.resolve(false),
  executeRedisCommand: (command: string, connectionId?: string) => contentAreaRef.value?.executeRedisCommand(command, connectionId) ?? Promise.resolve(false),
  isRedisConsoleReady: (connectionId: string) => contentAreaRef.value?.isRedisConsoleReady(connectionId) ?? false,
