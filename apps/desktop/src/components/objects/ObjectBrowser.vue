...
async function exportTableData(row: ObjectBrowserRow, format: "csv" | "xlsx" | "sql", columnInfos?: ColumnInfo[], headerMode: XlsxHeaderMode = "name", autoFilter = true, insertMode: SqlInsertMode = "batch") {
  ...
  try {
    if (isVictoriaMetrics.value) {
      ...
      if (format === "csv") {
        await api.exportQueryResultCsv(filePath, result.columns, forceCsvTextForTemporalColumns(result.rows, result.column_types ?? [], effectiveDatabaseType.value), settingsStore.editorSettings.csvQuoteMode);
      } else {
