...
import { buildCreateUserSql } from "@/lib/api";
import {
...
const createUserTablespace = ref("MAIN");
const showPassword = ref(false);

/** Generate a DM-compliant random password:
 *  - min 12 chars
 *  - at least one uppercase, one lowercase, one digit
 *  - not equal to the username
 */
function generateRandomPassword(username: string): string {
  ...
  // Ensure not same as username
  if (pw.toUpperCase() === username.toUpperCase()) {
    pw = "Db@" + pw.substring(4);
  }
  return pw;
}

/** Auto-fill password and tablespace when Tab is pressed after entering username */
function onUsernameTab() {
  const name = createUserUsername.value.trim().toUpperCase();
  if (!name) return;
  if (!createUserPassword.value.trim()) {
    createUserPassword.value = generateRandomPassword(name);
  }
  if (!createUserTablespace.value.trim() || createUserTablespace.value.trim() === "MAIN") {
    createUserTablespace.value = name;
  }
}
...
const canCreateUser = computed(() => {
  const config = props.node.connectionId ? connectionStore.getConfig(props.node.connectionId) : undefined;
  return props.node.type === "connection" && config?.db_type === "dameng";
});
...
function openCreateUserDialog() {
  ...
  createUserTablespace.value = "MAIN";
...
async function confirmCreateUser() {
  const node = props.node;
  // Uppercase DM names (DM is case-insensitive for identifiers without quotes)
  const rawUsername = createUserUsername.value.trim();
  const rawPassword = createUserPassword.value.trim();
  const rawTablespace = createUserTablespace.value.trim() || "MAIN";
  if (!rawUsername || !rawPassword || !node.connectionId) return;
  showCreateUserDialog.value = false;

  const dbType = databaseTypeForNode(node);
  const isDM = dbType === "dameng";

  // DM: force uppercase for username/tablespace (unquoted identifiers become uppercase)
  // Password is case-sensitive — NEVER uppercase it
  const username = isDM ? rawUsername.toUpperCase() : rawUsername;
  const password = rawPassword;
  const tablespace = isDM ? rawTablespace.toUpperCase() : rawTablespace;

  try {
    await connectionStore.ensureConnected(node.connectionId);
    const connId = node.connectionId;

    console.log("[createUser] username=", rawUsername, " password=", rawPassword, " tablespace=", rawTablespace);
    console.log("[createUser] DM normalized: username=", username, " password=", password, " tablespace=", tablespace);

    // For DM: auto-create tablespace if not exists
    if (isDM && tablespace !== "MAIN") {
      // DM view for tablespaces is DBA_TABLESPACES (Oracle-compatible)
      const checkSql = `SELECT TABLESPACE_NAME FROM DBA_TABLESPACES WHERE TABLESPACE_NAME = '${tablespace}'`;
      console.log("[createUser] check tablespace SQL:", checkSql);
      const checkResult = await api.executeQuery(connId, "", checkSql);
      const rows = checkResult?.rows || [];
      console.log("[createUser] tablespace check result rows:", rows.length);
      if (rows.length === 0) {
        // Create tablespace: 100MB with auto-extend
        const tsSql = `CREATE TABLESPACE "${tablespace}" DATAFILE '${tablespace}01.DBF' SIZE 100 AUTOEXTEND ON NEXT 10 MAXSIZE UNLIMITED;`;
        console.log("[createUser] create tablespace SQL:", tsSql);
        try {
          await api.executeQuery(connId, "", tsSql);
          console.log(`[createUser] Tablespace "${tablespace}" created`);
        } catch (e: any) {
          console.log("[createUser] first tablespace attempt failed:", e?.message || e);
          // Try without MAXSIZE if the first attempt failed
          const tsSql2 = `CREATE TABLESPACE "${tablespace}" DATAFILE '${tablespace}01.DBF' SIZE 100 AUTOEXTEND ON;`;
          console.log("[createUser] fallback tablespace SQL:", tsSql2);
          await api.executeQuery(connId, "", tsSql2);
        }
      }
    }

    // Create user
    const sql = await buildCreateUserSql(username, password, tablespace);
    console.log("[createUser] create user SQL:", sql);
    await api.executeQuery(connId, "", sql);
    toast(`用户「${username}」已创建`, 3000);
    await connectionStore.loadDatabases(connId, { force: true });
  } catch (e: any) {
      ...
      <div class="grid gap-3">
        <div class="grid gap-1.5">
          ...
          <Input
            ...
            placeholder="输入后 Tab 自动生成密码+表空间"
            @keydown.enter.prevent="confirmCreateUser"
            @keydown.tab="onUsernameTab"
          />
        </div>
        <div class="grid gap-1.5">
        ...
        <div class="grid gap-1.5">
          <label class="text-xs font-medium text-muted-foreground">表空间</label>
          <Input
            v-model="createUserTablespace"
            placeholder="Tab 自动填充为用户名"
            @keydown.enter.prevent="confirmCreateUser"
          />
        </div>
      </div>
      <DialogFooter>
        <Button variant="outline" @click="showCreateUserDialog = false">{{ t("dangerDialog.cancel") }}</Button>
        <Button :disabled="!createUserUsername.trim() || !createUserPassword.trim()" @click="confirmCreateUser">{{
          t("dangerDialog.confirm")
        }}</Button>
      </DialogFooter>
    </DialogContent>
