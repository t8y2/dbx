...
function offlineImportEntries(locale: Messages): Map<string, string> {
  const driverStore = locale.driverStore as Record<string, unknown>;
  return new Map(
    Object.entries(driverStore)
      .filter(([key, value]) => (key.startsWith("offlineImport") || key.startsWith("offlineJreImport")) && typeof value === "string")
...
describe("offline Agent import i18n parity", () => {
  const englishEntries = offlineImportEntries(en);

  it("declares the complete English message set", () => {
    expect([...englishEntries.keys()].sort()).toEqual(["offlineImportFailed", "offlineImportPartial", "offlineImportSuccess", "offlineImportWithJreSuccess", "offlineJreImportSuccess"]);
  });

  it.each(locales)("%s declares matching keys and placeholders", (_name, locale) => {
    const localeEntries = offlineImportEntries(locale);
