...
export default withEnglishFallback({
  ...
  settings: {
    ...
    mcpResultProtectionTitle: "結果データの保護",
    mcpResultProtectionBoundary: "MCPサーバーの更新が必要です。デスクトップの結果表示は維持されます。保護を有効にすると、再起動まで監査以外の診断ログが抑制されます。データベースの最小権限設定は引き続き必要です。フィルタリングですべての推論を防ぐことはできません。",
    mcpResultScope: "ポリシーの適用範囲",
    mcpResultGlobal: "グローバル既定値",
    mcpResultInherit: "個別設定を削除して継承",
    mcpResultOverride: "接続またはデータベースの個別設定を追加",
    mcpResultConnection: "接続",
    mcpResultDatabase: "データベース",
    mcpResultAllDatabases: "この接続のすべてのデータベース",
    mcpResultDatabaseBoundary:
      "データベースの個別設定には、ネイティブMySQL、PostgreSQL、SQL Serverの実際のカタログ名との完全一致が必要です。SQLiteは追加データベースのないmainのみ対応します。通常のテーブルを対象とする検証済みのSELECT構造のみを許可します。SHOW、通常の関数呼び出し、CTE、集合演算、データベースをまたぐSQL、初期化スクリプト、状態を保持するセッション、結合トランザクションは、保護を無効にした範囲でも拒否されます。",
    mcpResultEnabled: "結果保護を有効にする",
    mcpResultMode: "保護モード",
    mcpResultStrict: "厳格",
    mcpResultNameOnly: "列と値の照合",
    mcpResultStrictBoundary:
      "完全なメタデータを持つネイティブMySQL、PostgreSQL、SQLite、SQL Serverのみ対応します。検証済みの単一の基本テーブルから列を直接取得するか、*を使う場合のみ許可します。式、ビュー、生成列、継承やパーティションの親テーブル、接続初期化SQL、セッション、結合トランザクションは拒否されます。PostgreSQL/SQL Serverではスキーマを明示してください。ルールに一致しない自由形式のテキストは自動分類されません。",
    mcpResultNameOnlyBoundary: "列と値の照合による保護には限界があります。SQLによる変換や認識できない値の形式はルールを回避する可能性があります。このモードでは、可能な範囲でデータの出所を追跡します。",
    mcpResultRules: "ルール",
    mcpResultTemplate: "プリセットルールを追加",
    mcpResultAddRule: "ルールを追加",
    mcpResultRemoveRule: "ルールを削除",
    mcpResultNoRules: "ルールが設定されていません。ルール一覧が空の場合、保護を有効にできません。",
    mcpResultColumnPattern: "列名パターン（正規表現）",
    mcpResultAction: "処理",
    mcpResultAction_remove: "フィールドを削除",
    mcpResultAction_mask: "固定マスク",
    mcpResultAction_partial: "部分マスク",
    mcpResultAction_hash: "安定した鍵付きハッシュ",
    mcpResultAction_deny: "応答を拒否",
    mcpResultKeepPrefix: "先頭に残す文字数",
    mcpResultKeepSuffix: "末尾に残す文字数",
    mcpResultMatchers: "追加の照合条件",
    mcpResultTypePattern: "データ型パターン（正規表現）",
    mcpResultValuePattern: "値のパターン（正規表現）",
    mcpResultSchema: "スキーマ",
    mcpResultTable: "テーブル",
    mcpResultRuleId: "ルールID",
    mcpResultColumn: "列",
    mcpResultType: "データ型",
    mcpResultSample: "テスト値",
    mcpResultTest: "下書きをプレビュー",
    mcpResultNoMatches: "一致するルールはありません",
    mcpResultUnavailable: "MCPポリシーを利用できないか、保存中です。",
    mcpResultResources: "保護対象のリソース",
    mcpResultSearch: "グループ、接続、データベースを検索",
    mcpResultInactive: "現在適用されていない範囲",
    mcpResultPrecedence: "データベース > 接続 > 最も近い設定済みグループ > グローバル。各範囲はルール一覧全体を置き換えます。ルールは統合されません。",
    mcpResultSource: "適用中のポリシー：{source}",
    mcpResultState: "適用範囲の状態",
    mcpResultStateInherit: "上位から継承",
    mcpResultStateEnabled: "カスタム：有効",
    mcpResultStateDisabled: "明示的に無効",
    mcpResultCustomize: "継承したルールをカスタマイズ",
    mcpResultInheritConfirm: "この範囲の個別設定を削除して継承に戻しますか？",
    mcpResultEnabledExceptions: "この範囲の配下に有効な例外が{count}件あります",
    mcpResultEmptyEnabled: "有効な範囲には少なくとも1つのルールが必要です。",
    mcpResultClearRules: "すべてのルールを削除",
    mcpResultClearConfirm: "この範囲のすべてのルールを削除しますか？",
    mcpResultPreviewFormat: "テスト値の形式",
    mcpResultPreviewText: "テキスト",
    mcpResultPreviewJson: "JSON",
    mcpResultPreviewInvalidJson: "有効なJSON値を入力してください。",
    mcpResultPreviewFailed: "プレビューに失敗しました。ポリシーを確認して再試行してください。",
    mcpResultPreviewBefore: "保護前",
    mcpResultPreviewAfter: "MCP出力",
    mcpResultPreviewStatusDisabled: "保護は無効です",
    mcpResultPreviewStatusUnchanged: "変更なし",
    mcpResultPreviewStatusProtected: "保護済み",
    mcpResultPreviewStatusRemoved: "フィールド削除済み",
    mcpResultPreviewStatusDenied: "応答拒否",
    mcpResultDatabaseUnsupported: "この接続構成ではデータベース単位の個別設定を利用できません。",
    mcpResultDatabaseLoadFailed: "データベース一覧を読み込めませんでした。更新して再試行してください。",
    mcpResultRefreshDatabases: "データベース一覧を更新",
