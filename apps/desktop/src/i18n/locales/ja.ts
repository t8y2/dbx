...
export default withEnglishFallback({
  ...
  settings: {
    ...
    sqlVariableSyntaxDescription: "SQL 実行前の変数・プレースホルダー置換方法をデータベース種別ごとに選択します。構文は既定で有効ですが、引用符内の置換はオプトインです。",
    ...
    sqlVariableSyntax_replaceInsideQuotes: "引用符内の波括弧プレースホルダーを置換",
    sqlVariableSyntax_replaceInsideQuotesDescription: "引用符付き文字列や識別子内の ${name} と #{name} も検出します。値はエスケープ済みテキストとして挿入され、既定では無効です。",
    sqlVariableSyntax_ansiQuotes: "ANSI_QUOTES モード",
    sqlVariableSyntax_ansiQuotesDescription: "MySQL、Doris、または StarRocks セッションで ANSI_QUOTES を使用する場合に有効にし、二重引用符内のプレースホルダーを文字列ではなく識別子として扱います。",
    sqlVariableSyntax_noBackslashEscapes: "NO_BACKSLASH_ESCAPES モード",
    sqlVariableSyntax_noBackslashEscapesDescription: "MySQL、Doris、または StarRocks セッションで NO_BACKSLASH_ESCAPES を使用する場合に有効にし、バックスラッシュを二重化せず文字どおりに保持します。",
