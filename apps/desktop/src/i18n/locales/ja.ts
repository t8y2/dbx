...
export default withEnglishFallback({
  ...
  transfer: {
    ...
    batchSize: "バッチサイズ",
    dropTargetBeforeCreate: "作成前にターゲットテーブルを削除",
    dropTargetBeforeCreateHint: "ターゲットテーブルをバックアップにリネームし、ソース構造から再構築し、データを挿入した後、バックアップを削除します。いずれかの手順が失敗した場合、バックアップテーブルは保持されます。",
    dropTargetBeforeCreateWarning: "この操作は、再構築前に既存のターゲットテーブルを一時的にリネームします（__dbx_bak_ サフィックスを追加）。転送が成功すると、バックアップテーブルは自動的に削除されます。",
    dropTargetBeforeCreateDanger: "{count} 個のターゲットテーブルに対して DROP と CREATE が実行されます",
    dropTargetBeforeCreateProductionBlock: "本番データベースでこの操作はブロックされています。続行するには、接続設定で本番保護を無効にしてください。",
    start: "転送開始",
