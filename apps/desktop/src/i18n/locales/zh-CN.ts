...
export default withEnglishFallback({
  ...
  connection: {
    ...
    sqlServerLegacyUnencryptedRetry: "尝试旧版非加密模式",
    sqlServerLegacyUnencryptedModeEnabled: "已启用 SQL Server 旧版非加密模式。",
    sqlServerLegacyUnencryptedModeHint: "当前 SQL Server 连接使用类似 encrypt=false 的旧版非加密模式，仅适用于服务端允许非加密或仅登录阶段加密的场景。若服务端强制 TLS 1.0 加密，此模式仍会失败。请仅在可信内网、VPN 或 SSH 隧道中使用。",
