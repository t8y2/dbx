...
import { buildConnectionUrlCopy, connectionSupportsUrlCopy, connectionUrlCopyFormats, type ConnectionUrlCopyConfig } from "@/lib/connection/connectionUrlBuilder";
...
describe("connectionUrlCopyFormats", () => {
  it("lists all seven formats for a PostgreSQL connection with a stored password", () => {
    expect(connectionUrlCopyFormats(config({}))).toEqual(["url", "urlNoPassword", "jdbcUrl", "jdbcUrlWithCredentials", "hostPort", "dsn", "psqlCommand"]);
  });

  it("drops the no-password variants when nothing redactable differs", () => {
    const formats = connectionUrlCopyFormats(config({ password: "" }));
    expect(formats).not.toContain("urlNoPassword");
    expect(formats).not.toContain("jdbcUrlWithCredentials");
    expect(formats).toContain("url");
    expect(formats).toContain("jdbcUrl");
  });
...
describe("buildConnectionUrlCopy standard URL", () => {
  it("builds a PostgreSQL URL with credentials", () => {
    expect(buildConnectionUrlCopy(config({}), "url")).toBe("postgresql://app_user:secret@db.example.com:5432/appdb");
  });

  it("omits the password for the no-password variant", () => {
    expect(buildConnectionUrlCopy(config({}), "urlNoPassword")).toBe("postgresql://app_user@db.example.com:5432/appdb");
  });

  it("percent-encodes special characters in credentials and database", () => {
    const text = buildConnectionUrlCopy(config({ username: "user@corp", password: "p@ss:w/ord#", database: "app db" }), "url");
  ...
  it("appends url_params and adds sslmode when ssl is enabled", () => {
    expect(buildConnectionUrlCopy(config({ url_params: "application_name=svc" }), "url")).toBe("postgresql://app_user:secret@db.example.com:5432/appdb?application_name=svc");
    expect(buildConnectionUrlCopy(config({ ssl: true, url_params: "application_name=svc" }), "url")).toBe("postgresql://app_user:secret@db.example.com:5432/appdb?application_name=svc&sslmode=require");
    expect(buildConnectionUrlCopy(config({ ssl: true, url_params: "sslmode=verify-full" }), "url")).toBe("postgresql://app_user:secret@db.example.com:5432/appdb?sslmode=verify-full");
  });

  it("honours the database override from a database tree node", () => {
    expect(buildConnectionUrlCopy(config({}), "url", { database: "reporting" })).toBe("postgresql://app_user:secret@db.example.com:5432/reporting");
  });

  it("uses rediss:// when TLS is enabled for Redis", () => {
    expect(buildConnectionUrlCopy(config({ db_type: "redis", host: "cache.example.com", port: 6379, username: "", database: "0", ssl: true }), "url")).toBe("rediss://:secret@cache.example.com:6379/0");
  });

  it("maps the MySQL family profiles to their own schemes", () => {
    expect(buildConnectionUrlCopy(config({ db_type: "mysql", port: 3306 }), "url")).toBe("mysql://app_user:secret@db.example.com:3306/appdb");
    expect(buildConnectionUrlCopy(config({ db_type: "mysql", port: 3306, driver_profile: "mariadb" }), "url")).toBe("mariadb://app_user:secret@db.example.com:3306/appdb");
    expect(buildConnectionUrlCopy(config({ db_type: "mysql", port: 2883, driver_profile: "oceanbase" }), "url")).toBe("oceanbase://app_user:secret@db.example.com:2883/appdb");
    expect(buildConnectionUrlCopy(config({ db_type: "doris", port: 9030 }), "url")).toBe("mysql://app_user:secret@db.example.com:9030/appdb");
  });

  it("keeps multi-host endpoints verbatim without an extra port suffix", () => {
    const text = buildConnectionUrlCopy(config({ db_type: "gaussdb", host: "h1.example.com:5432,h2.example.com:5432" }), "url");
    expect(text).toBe("postgresql://app_user:secret@h1.example.com:5432,h2.example.com:5432/appdb");
  });

  it("brackets bare IPv6 hosts", () => {
    expect(buildConnectionUrlCopy(config({ host: "fe80::1" }), "url")).toBe("postgresql://app_user:secret@[fe80::1]:5432/appdb");
  });

  it("passes explicit connection strings through and redacts them for the no-password variant", () => {
    ...
    expect(buildConnectionUrlCopy(jdbcConfig, "url")).toBe(jdbcConfig.connection_string);
    expect(buildConnectionUrlCopy(jdbcConfig, "urlNoPassword")).toBe("jdbc:dremio:direct=drill.example.com:31010;user=analyst;password=***");
  });
});
...
describe("buildConnectionUrlCopy JDBC URL", () => {
  ...
  it("returns the explicit connection string for jdbc-type connections", () => {
...
describe("buildConnectionUrlCopy host:port, DSN and psql", () => {
  ...
  it("builds a libpq key=value DSN", () => {
    expect(buildConnectionUrlCopy(config({}), "dsn")).toBe("host=db.example.com port=5432 user=app_user password=secret dbname=appdb");
    expect(buildConnectionUrlCopy(config({ ssl: true }), "dsn")).toBe("host=db.example.com port=5432 user=app_user password=secret dbname=appdb sslmode=require");
    expect(buildConnectionUrlCopy(config({ password: "has space" }), "dsn")).toContain("password='has space'");
  });
