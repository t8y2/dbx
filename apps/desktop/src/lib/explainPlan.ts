...
export function parseDamengExplainText(planText: string): ParsedExplainPlan {
  ...
  for (const { indent, line } of operatorLines) {
    ...
    const childIndex = parentStack.length === 0
      ? rootNodes.length + 1
      : parentStack[parentStack.length - 1].children.length + 1;
    const id = parentStack.length === 0
      ? String(childIndex)
      : `${parentStack[parentStack.length - 1].id}.${childIndex}`;
...
function parseDamengPlanLine(line: string): DamengOpInfo | null {
  ...
  const semiParts = afterBracket.split(";").map((s) => s.trim()).filter(Boolean);
