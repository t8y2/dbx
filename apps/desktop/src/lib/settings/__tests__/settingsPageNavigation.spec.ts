...
describe("settings page navigation", () => {
  it("keeps query, settings, and driver-manager surfaces mutually exclusive", () => {
    expect(appSource).toContain('type MainContentSurface = "query" | "settings" | "driverStore" | "dataView";');
    expect(appSource).toMatch(/function activateMainContentSurface\(surface: MainContentSurface\) \{\s*settingsStore\.settingsPageActive = surface === "settings";\s*driverStoreActive\.value = surface === "driverStore";\s*dataViewActive\.value = surface === "dataView";/);
