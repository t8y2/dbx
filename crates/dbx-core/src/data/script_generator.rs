...
mod tests {
    ...
    fn generate_enhanced_sync_sql_with_idempotent() {
        let schema_diff =
            SchemaDiffPreparation { sync_sql: "CREATE TABLE test (id INT);".to_string(), ..empty_schema_diff() };
    ...
    fn generate_dml_clean_integration() {
        ...
        let prep = SchemaDiffPreparation { diffs: vec![table_diff], ..empty_schema_diff() };
