...
pub(crate) fn generate_insert_typed_sql_batches(
    ...
    while start < rows.len() {
        ...
        let mut end = max_end;
        let mut accepted =
            generate_insert_typed(columns, column_types, &rows[start..max_end], table, schema, db_type, catalog);

        if accepted.len() > max_sql_bytes && max_end > start + 1 {
            end = start + 1;
            accepted = generate_insert_typed(columns, column_types, &rows[start..end], table, schema, db_type, catalog);
            let mut low = start + 2;
            let mut high = max_end;
            while low <= high {
                let candidate_end = low + (high - low) / 2;
                let candidate = generate_insert_typed(
                    columns,
                    column_types,
                    &rows[start..candidate_end],
                    table,
                    schema,
                    db_type,
                    catalog,
...
SELECT 1 FROM dual"#
    ...
    async fn duckdb_transfer_columns_use_requested_schema() {
        ...
        let columns =
            get_columns_for_transfer(&state, "duckdb-1", "duckdb-1", "main", "analytics", "items", None).await.unwrap();
