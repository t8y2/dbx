...
fn postgres_copy_transfer_sql(
    ...
    let target_col_list =
        col_names.iter().map(|c| transfer_column_identifier(c, target_db_type, quoting)).collect::<Vec<_>>().join(", ");
  ...
  PRIMARY KEY (`id`)
    ...
    fn postgres_copy_transfer_sql_wraps_select_and_targets_columns() {
        ...
        let (copy_out, copy_in) = postgres_copy_transfer_sql(
            ...
            &DatabaseType::Postgres,
            None,
            TransferColumnQuoting::new(false, None),
        );
        ...
        // With target quoting disabled, openGauss folds simple lowercase column
        // names but keeps quotes on anything case-sensitive or reserved — the
        // same rule the multi-row INSERT fallback uses (t8y2/dbx#6283).
