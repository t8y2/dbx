...
mod tests {
    ...
    async fn attached_database_metadata_uses_attached_schema() {
        ...
        assert!(crate::metadata::sqlite_ddl(&pool, "analytics.db", "child")
            .await
...
pub fn sqlite_quote_ident(value: &str) -> String {
    format!("\"{}\"", value.replace('"', "\"\""))
}

pub fn sqlite_quote_schema_ident(value: &str) -> String {
    sqlite_quote_ident(sqlite_schema_name(value))
}

pub fn sqlite_quote_schema_ident_for_connection(conn: &Connection, value: &str) -> Result<String, String> {
...
pub async fn execute_query_with_max_rows_progress(
    ...
    if let Some(worker) = pool.worker() {
        ...
        return crate::execution::wait_for_query_opt(None, timeout, worker.query(&sql, max_rows)).await;
    }
    let pool = pool.clone();
