...
/// `estimated_plan_strategy_matches_capability_sql_format_and_acquisition` guards the pairing.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
...
/// The execution contract used to acquire an estimated plan.
///
/// This describes the acquisition path only; connection and driver execution
/// remain owned by `dbx-core`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EstimatedPlanAcquisition {
    /// Execute the SQL generated by `build_explain_sql`.
    GeneratedSql,
    /// Ask the connected driver's existing explain facility for a plan.
    DriverNative,
    /// Capture SHOWPLAN on a dedicated SQL Server client session.
    SqlServerShowPlanSession,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EstimatedPlanSqlGeneration {
    Postgres,
    Mysql,
    Plain,
    Oracle,
...
/// One complete Estimated Plan contract for a database dialect.
///
/// Capability, SQL generation, payload encoding and Plugin Host acquisition
/// all derive from this mapping.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct EstimatedPlanStrategy {
    sql_generation: EstimatedPlanSqlGeneration,
    acquisition: EstimatedPlanAcquisition,
    format: EstimatedPlanFormat,
}

impl EstimatedPlanStrategy {
    pub const fn acquisition(self) -> EstimatedPlanAcquisition {
        self.acquisition
    }

    pub const fn format(self) -> EstimatedPlanFormat {
        self.format
    }
}
...
pub fn build_explain_sql(options: ExplainSqlOptions) -> ExplainSqlBuildResult {
    let Some(strategy) = estimated_plan_strategy(options.database_type) else {
    ...
    let sql = match strategy.sql_generation {
        // ANALYZE executes the statement; the safety gate above has already
        // limited the source to the existing read-only subset.
        EstimatedPlanSqlGeneration::Postgres if options.analyze == Some(true) => {
        ...
        EstimatedPlanSqlGeneration::SqlServer => {
            format!("SET SHOWPLAN_XML ON;\nGO\n{source}\nGO\nSET SHOWPLAN_XML OFF;")
        }
        EstimatedPlanSqlGeneration::Mysql if options.format == Some(ExplainFormat::Standard) => {
        ...
        EstimatedPlanSqlGeneration::Mysql | EstimatedPlanSqlGeneration::Json => {
...
/// The estimated-plan contract for a database, if one is currently supported.
pub fn estimated_plan_strategy(database_type: Option<DatabaseType>) -> Option<EstimatedPlanStrategy> {
    ...
    let strategy = match database_type? {
        ...
        DatabaseType::Questdb => {
            EstimatedPlanStrategy { sql_generation: Plain, acquisition: GeneratedSql, format: Text }
        }
        DatabaseType::Dameng => {
            EstimatedPlanStrategy { sql_generation: Plain, acquisition: DriverNative, format: Text }
        }
        DatabaseType::Oracle => {
            EstimatedPlanStrategy { sql_generation: Oracle, acquisition: DriverNative, format: Text }
        }
        DatabaseType::OceanbaseOracle => EstimatedPlanStrategy {
            sql_generation: EstimatedPlanSqlGeneration::Json,
            acquisition: GeneratedSql,
            format: Json,
        },
        DatabaseType::SqlServer => {
            EstimatedPlanStrategy { sql_generation: SqlServer, acquisition: SqlServerShowPlanSession, format: Xml }
        }
        // The former MongoDB SQL-generation arm was unreachable behind the old
        // capability gate; it remains unsupported and has no Estimated Plan strategy.
        _ => return None,
    };
    Some(strategy)
}

/// Payload encoding defined by the estimated-plan strategy for this dialect.
/// Keep the historical JSON fallback for unsupported/unspecified dialects;
/// callers must check capability separately before using it.
pub fn estimated_plan_format(database_type: Option<DatabaseType>) -> EstimatedPlanFormat {
    estimated_plan_strategy(database_type).map_or(EstimatedPlanFormat::Json, EstimatedPlanStrategy::format)
}

pub fn supports_explain_plan(database_type: Option<DatabaseType>) -> bool {
    estimated_plan_strategy(database_type).is_some()
}
...
mod tests {
    ...
    fn ignores_analyze_for_every_engine_but_postgres_and_sqlserver() {
        for (db_type, expected) in [
            ...
            // MongoDb is rejected by the strategy capability gate; its historical
            // SQL-generation arm remains unreachable.
    ...
    #[test]
    fn estimated_plan_strategy_matches_capability_sql_format_and_acquisition() {
        use EstimatedPlanAcquisition::{DriverNative, GeneratedSql, SqlServerShowPlanSession};
        use EstimatedPlanFormat::{Json, Text, Xml};
        ...
        for (database_type, expected_sql, expected_format, expected_acquisition) in cases {
            let strategy = estimated_plan_strategy(Some(database_type))
                .unwrap_or_else(|| panic!("{database_type:?} must have a complete Estimated Plan strategy"));
            ...
            assert_eq!(strategy.format(), expected_format, "{database_type:?}");
            assert_eq!(estimated_plan_format(Some(database_type)), expected_format, "{database_type:?}");
            ...
            assert!(result.ok, "{database_type:?}: {:?}", result.reason);
            assert_eq!(result.sql.as_deref(), Some(expected_sql), "{database_type:?}");
        }
        ...
        for database_type in [
            ...
            Some(DatabaseType::StarRocks),
        ] {
            assert!(estimated_plan_strategy(database_type).is_none(), "{database_type:?}");
            assert!(!supports_explain_plan(database_type), "{database_type:?}");
            assert_eq!(
                build_explain_sql(ExplainSqlOptions {
                    database_type,
                    format: None,
        ...
        // Preserve the legacy fallback for callers that ask for a format without
        // first checking support; it does not enable plan capability.
        assert_eq!(estimated_plan_format(None), Json);
        assert_eq!(estimated_plan_format(Some(DatabaseType::Redis)), Json);
    }
    ...
    fn builds_mysql_json_explain_and_rejects_unsafe_sql() {
        ...
        for sql in [
            ...
            assert_eq!(
                ...
                ExplainSqlBuildResult { ok: false, sql: None, reason: Some("unsafe".to_string()) },
                "{sql} must remain blocked by the explain safety gate"
            );
        }
