...
async fn main() {
    ...
    let api = Router::new()
        ...
        // Kafka
        .route("/kafka/list-topics", post(routes::kafka::list_topics))
        .route("/kafka/describe-topic", post(routes::kafka::describe_topic))
        .route("/kafka/fetch-messages", post(routes::kafka::fetch_messages))
        .route("/kafka/produce-message", post(routes::kafka::produce_message))
        .route("/kafka/get-topic-message-count", post(routes::kafka::get_topic_message_count))
        .route("/kafka/list-brokers", post(routes::kafka::list_brokers))
        .route("/kafka/list-acls", post(routes::kafka::list_acls))
        .route("/kafka/list-consumer-groups", post(routes::kafka::list_consumer_groups))
        .route("/kafka/describe-consumer-group", post(routes::kafka::describe_consumer_group))
        .route("/kafka/create-topic", post(routes::kafka::create_topic))
        .route("/kafka/delete-topic", post(routes::kafka::delete_topic))
        .route("/kafka/schema-registry/list-subjects", post(routes::kafka::schema_registry_list_subjects))
        .route("/kafka/schema-registry/list-versions", post(routes::kafka::schema_registry_list_versions))
        .route("/kafka/schema-registry/get-schema", post(routes::kafka::schema_registry_get_schema))
        .route("/kafka/decode-payload", post(routes::kafka::decode_payload))
        .route("/kafka/tail/start", post(routes::kafka::tail_start))
        .route("/kafka/tail/stop", post(routes::kafka::tail_stop))
        .route("/kafka/tail/progress/{tail_id}", get(routes::kafka::tail_progress))
        .route("/kafka/topic-count/start", post(routes::kafka::topic_count_start))
        .route("/kafka/topic-count/stop", post(routes::kafka::topic_count_stop))
        .route("/kafka/topic-count/progress/{session_id}", get(routes::kafka::topic_count_progress))
        // MongoDB
