...
`DBX_WEB_PASSWORD` is the account's Web login password; set `DBX_WEB_USERNAME` when the account name is not `admin`. In Web mode, connections, permissions, drivers, and file paths belong to the server environment. `dbx open` still targets a local Desktop bridge and should not be treated as a remote Web automation API.
