...
| Drop Target Before Create | Renames existing target tables to backup (`__dbx_bak_` prefix), rebuilds from source structure, inserts data, then drops backup on success. Any failure preserves the backup table for manual recovery. Blocked on production databases. Available for MySQL, PostgreSQL, Oracle, SQL Server, DM, OceanBase-Oracle, KingBase, GaussDB, OpenGauss, KWDB, GoldenDB, SQLite, DuckDB, and Cloudflare D1. |
