...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    | 参数 | 默认值 | 用途 |
    | --- | --- | --- |
    | `DBX_PASSWORD` | 未设置 | 预配置 Web 登录账号的密码，生产环境建议设置 |
    | `DBX_USERNAME` | `admin` | 预配置账号的用户名（与 `DBX_PASSWORD` 搭配使用） |
    | `DBX_DISABLE_PASSWORD` | `false` | 设为 `true` 或 `1` 可关闭登录保护 |
    | `DBX_COOKIE_SECURE` | `false` | 设为 `true` 或 `1` 可让会话 Cookie 带有 `Secure` 属性（HTTPS 部署） |
    | `DBX_SESSION_IDLE_TIMEOUT_MINUTES` | 未设置 | 空闲超过该分钟数后会话过期；`0` 或不设置时保持到退出登录或重启 |
    | `DBX_DATA_DIR` | `/app/data` | 持久化数据目录 |
    | `DBX_PORT` | `4224` | 容器内监听端口 |
    | `DBX_PUBLIC_BASE_PATH` | `/` | 子路径部署，例如 `/dbx` |

    未设置 `DBX_PASSWORD` 时，首次访问会显示初始化页面，用于创建首个用户名和密码，该账号即为管理员。管理员可在 **设置 → 安全** 中创建和管理更多账号。账号仅用于登录鉴权：数据库连接与应用设置都保存在服务端，并在所有已登录账号之间共享。
