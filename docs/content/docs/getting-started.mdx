...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    | --- | --- | --- |
    | `DBX_PASSWORD` | Not set | Password of the pre-configured web login account; recommended for production |
    ...
    | `DBX_COOKIE_SECURE` | `false` | Set to `true` or `1` to mark session cookies `Secure` (HTTPS deployments) |
    | `DBX_SESSION_IDLE_TIMEOUT_MINUTES` | Not set | Expire sessions idle longer than this; `0`/unset keeps them until logout or restart |
    ...
    Without `DBX_PASSWORD`, the first visit shows a setup page that creates the initial username and password — that first account is the admin. Admin accounts can create and manage additional accounts under **Settings → Security**. Accounts gate login only: database connections and app settings are stored server-side and stay shared across all signed-in accounts.
