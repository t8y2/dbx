...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Linux">
    ...
    On Kylin V10, UnionTech UOS, and similar Chinese desktop distributions, install from the [Spark App Store](https://spk-resolv.spark-app.store/?spk=spk://store/development/dbx) and choose the **APM build**. AmberPM provides a compatible runtime that reduces install/startup failures caused by distro dependency drift; updates are also available from the Spark App Store client.

    <Callout type="warn">
      The APM build runs DBX inside that compatibility runtime. If you select the host Java for Agent/JDBC drivers, prefix the path with `/host` — enter `/host/usr/bin/java` instead of `/usr/bin/java`.
    </Callout>
