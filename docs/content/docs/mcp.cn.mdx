...
1. 选择全局默认，或添加连接、数据库覆盖。数据库名称精确匹配。数据库设置覆盖连接设置，连接设置覆盖全局设置，规则列表不合并；删除覆盖后恢复继承。
2. 手动添加规则，或显式添加凭据、手机号、身份证号和邮箱模板。确认字段模式及处理动作后，再启用该作用域。
3. 使用合成样例测试，启用保护并保存。预览只返回命中字段、规则 ID 和动作，不返回样例值；未保存草稿不会改变生效策略。

数据库覆盖仅支持原生 MySQL、PostgreSQL、SQL Server 和 SQLite（仅限无附加数据库的 `main`）。请求及配置的数据库名称必须与实时目录精确匹配，不接受别名或大小写变体（SQLite 请求中的 `main` 大小写会统一处理），并要求账号能读取数据库目录。URL 参数不得重定向数据库或通过 MySQL `sessionVariables` 执行自定义初始化 SQL。这些连接拒绝跨库 SQL、初始化脚本、已有有状态会话及合并事务批次，包括关闭保护或仅名称模式的作用域；驱动支持时，查询使用独立且绑定到目标库的临时会话。这些连接中停用保护的数据库作用域仍会清理元数据中的值属性，并拒绝存储过程源码。其他引擎（包括 MongoDB）应使用全局或连接级策略；Redis 在存在数据库覆盖时拒绝数据操作。
