...
导入工具只支持本地 DBX Desktop/MCP 模式；在 Web 模式中会稳定返回 `IMPORT_UNSUPPORTED_IN_WEB_MODE_V1`。必须用平台路径分隔符在 `DBX_MCP_IMPORT_ROOTS` 中配置一个或多个允许目录。文件路径必须是绝对路径，解析后仍位于允许目录内，是普通且非符号链接的文件，扩展名受支持，并且没有超过大小限制。

`dbx_prepare_table_import` 会把连接、已批准模板版本、规范路径、大小、修改时间、SHA-256、解析参数、字段映射和 staging 目标固化为不可变摘要。计划 30 分钟后过期，且只能启动一次。`dbx_start_table_import` 在后台启动前会重新核对完整文件身份。导入目标仅限 `DBX_MCP_IMPORT_STAGING_SCHEMAS` 中的 PostgreSQL Schema（默认为 `staging`）；发布到业务 Schema 仍由调用方通过独立治理流程完成。

Milvus 工具只接受 allowlist 中的集合、过滤字段和输出字段。检索请求必须提供 `active_at`；服务端始终追加 `approval_status == "approved"` 以及 `effective_from`/`effective_to` 生效时间过滤，并可选追加精确 `semantic_version`。JSONL upsert 需要“数据读写”权限；删除只能按精确 `semantic_batch_id` 执行，并要求“完全访问”权限。通过 `dbx_execute_query` 提交的通用向量 REST 请求也会按端点分类，因此写入或破坏性 `POST` 不能绕过 MCP 策略。
...
仅依赖生效日期选择活动语义卡时可以省略 `semantic_version`，但 `active_at` 仍为必填。`embedding` 的实际长度必须等于 `DBX_MCP_VECTOR_DIMENSION`；上面数组内的注释只是文档缩写，不是可直接提交的工具参数。
