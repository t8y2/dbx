...
仅依赖生效日期选择活动语义卡时可以省略 `semantic_version`，但 `active_at` 仍为必填。v1 的 `embedding` 必须恰好包含 1024 个有限数值；上面数组内的注释只是文档缩写，不是可直接提交的工具参数。
