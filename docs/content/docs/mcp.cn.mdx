...
在 **设置 → MCP → 结果数据保护** 中配置服务端过滤，在查询结果进入 AI 客户端之前执行。此功能默认关闭。启用前须同时更新 DBX 和所有独立 MCP 服务端，旧版 MCP 二进制不会执行此策略。

1. 选择全局默认或已授权的分组、连接、数据库。资源选择器与 MCP 授权范围联动，包括通过分组获得授权的连接。未授权资源的已有配置仍可清理，配置保护不会授予访问权限。
2. 选择继承上级、自定义开启或显式关闭。优先级为 **数据库 > 连接 > 最近的已配置分组 > 全局**，子分组设置优先于父分组。编辑器显示当前生效状态及来源；规则列表整体替换上级，不合并。
3. 手动添加规则，或添加凭据、手机号、身份证号和邮箱预设规则。预设只填入可编辑的规则，没有独立的模板绑定。关闭保留本级规则以便再次开启；恢复继承删除本级覆盖，清空规则本身不等同于关闭。开启的策略必须至少有一条规则。
4. 以文本或 JSON 输入合成样例进行预览，再保存。文本保留前导零及长标识符。预览使用当前草稿的有效策略和实际查询相同的过滤引擎，不会暗中开启已关闭的策略，也不会保存变更。结果展示处理后的样例及命中规则，或明确标记未命中、已关闭、字段已移除、响应被拒绝。删除和拒绝时不返回样例值。预览不查询业务数据，也不把样例值写入审计日志。

例如全局开启、测试分组显式关闭时，继承该分组的连接返回未过滤值，但单独开启的连接或数据库仍受保护。服务端每次请求根据已保存的侧边栏布局解析分组归属，新增或移动连接会随之改变继承，不会批量复制策略。配置分组策略后，分组归属读取失败时拒绝返回数据。关闭结果过滤不放宽访问授权，也不解除下述数据库作用域边界。

数据库覆盖仅支持原生 MySQL、PostgreSQL、SQL Server 和 SQLite（仅限无附加数据库的 `main`）。请求及配置的数据库名称必须与实时目录精确匹配，不接受别名或大小写变体（SQLite 请求中的 `main` 大小写会统一处理），并要求账号能读取数据库目录。URL 参数不得重定向数据库或通过 MySQL `sessionVariables` 执行自定义初始化 SQL。这些连接拒绝跨库 SQL、初始化脚本、已有有状态会话及合并事务批次，包括关闭保护或仅名称模式的作用域；驱动支持时，查询使用独立且绑定到目标库的临时会话。这些连接中停用保护的数据库作用域仍会清理元数据中的值属性，并拒绝存储过程源码。其他引擎（包括 MongoDB）应使用全局、分组或连接级策略；Redis 在存在数据库覆盖时拒绝数据操作。

配置任一数据库覆盖后，SQL 工具仅接受使用普通表名的可核验 `SELECT` 结构（或仅含字面量的查询）。`SHOW`、`CALL`、DML/DDL、`TABLE`、CTE、集合运算、派生表/表函数、`SELECT INTO`、普通函数调用（含谓词或排序中的调用）和可执行注释均被拒绝，包括停用保护或仅名称模式的作用域。`TRIM`、`SUBSTRING` 等内建表达式语法仍受所选保护模式的规则约束。这一检查验证显式数据库引用；停用保护或仅名称模式不验证视图、同义词的底层来源，仍须通过数据库权限约束这些来源。

规则可组合字段名、数据库报告的数据类型、值格式的正则表达式，以及 schema/table 限定；配置的条件必须全部满足。正则匹配不区分大小写。字段规则同时检查直接来源列和输出别名。值规则处理整个单元格，不做局部字符串替换。缺少规则要求的数据类型或无法确认限定来源时，拒绝返回。

| 动作 | 结果 |
| --- | --- |
| 删除字段 | 删除整个匹配列，或匹配的 JSON 属性 |
| 固定掩码 | 将非空值替换为 `[REDACTED]` |
| 部分掩码 | 保留指定数量的首尾字符；过短值整体隐藏 |
| 稳定哈希 | 使用本地生成并持久化的密钥输出 HMAC-SHA256 化名 |
| 拒绝返回 | 返回不含查询数据的错误 |

拒绝和删除优先于其他动作，其余转换取第一条命中规则。匹配基于原始值。值规则的删除动作只要命中任意返回行，就删除所有行中的该列。JSON 容器不允许部分掩码，应整体掩码/删除或使用嵌套字段规则。保持哈希密钥私密且稳定，才能保持化名一致；哈希仍暴露相等关系，不等同于匿名数据。

### 严格模式与仅名称模式

**严格模式**只接受单个已验证基础表的直接列（含 `phone AS remark` 等别名）或单独的无修饰 `*`。服务端验证原生元数据，将执行 SQL 绑定到已确认的表，拒绝视图、生成列/计算列、包含继承/分区子关系的 PostgreSQL 父表、联表、CTE、子查询、UNION、投影中的函数/拼接/JSON 提取、未知驱动来源及有歧义的元数据。原生 MySQL、PostgreSQL、SQLite 和 SQL Server 在元数据完整时支持；兼容性降级、Agent/JDBC 和外部驱动不能提供此保证。PostgreSQL 和 SQL Server 查询需限定 schema 或配置默认 schema；MySQL 库名及 SQL Server schema 名称必须与目录精确匹配。严格模式不支持有状态会话、连接初始化脚本（含 MySQL URL 的 `sessionVariables`）和多语句事务批次。

**仅名称模式**是明确较弱的边界：尽可能追踪简单直接别名，但表达式、视图、函数、变换后的值及重命名仍可能绕过字段规则，不能用于抵御任意 SQL 变换。带 schema/table 限定的规则在来源无法确认时仍拒绝返回。

两种模式都递归处理 JSON 对象、数组和 JSON 编码的对象/数组字符串。严格模式拒绝疑似 JSON 但无法解析的值和过深嵌套。普通自由文本不会自动识别：`remark = '联系 13812345678'` 不会被 `phone` 字段名规则处理。请增加对应值格式规则，或直接掩码/删除自由文本列。模板不会自动启用宽泛的值检测。

### 返回通道与运维

`dbx_execute_query` 和 `dbx_execute_batch` 的每个结果都使用同一份过滤数据生成文本 `content` 与 `structuredContent`。messages、原始响应附加内容和数据库错误原文不会返回。元数据工具去除默认值、注释等可能携带数据的属性；受保护作用域不能获取存储过程源码。MongoDB 支持仅名称过滤；Redis 在作用域受保护时拒绝返回数据。`dbx_execute_and_show` 保留现有桌面权限，MCP 只收到状态。

规则无效、策略不可用、来源不支持或处理失败时，返回不含数据的错误，不回退到原始结果。保存失败保留先前策略。通过 MCP 复制连接时保留结果保护覆盖。过滤不会放宽连接/数据库/工具白名单、只读、指定 SQL 确认、生产保护或数据库账号权限。

一旦使用启用的持久化策略，DBX 就在当前宿主进程中抑制非审计诊断日志，直到重启；之后关闭策略也不会恢复该进程的诊断输出，以覆盖可能晚于请求结束的驱动后台任务。预览不会触发此抑制。审计只包含连接/数据库标识、经过验证的规则 ID 和动作，不含 SQL、样例/单元格值或动态列名/JSON 键。数据库服务端和第三方基础设施日志仍由各自管理员控制。

<Callout type="warn">结果过滤不替代数据库最小权限。它不能识别所有敏感自由文本、追踪此前已复制到普通列的数据，也不保证在允许任意 SQL 时阻止所有统计推断或副作用。请继续限制凭据权限、SQL 权限和开放工具。</Callout>
