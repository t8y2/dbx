...
DBX MCP 定义 25 个工具；日常默认注册 22 个，只有显式无作用域维护模式才额外注册 3 个连接管理工具：
...
连接解析始终先取运行时 scope 与全局 `allowed_connection_ids` 的交集。`connection_id` 按 ID 精确解析；`connection_name` 可按 ID 精确解析，也可在全部有效候选中按名称大小写不敏感解析。只有交集内恰好一个连接且没有 selector 时才允许隐式使用；多个 `DBX_MCP_SCOPE_CONNECTION_IDS` 同时有效时，省略 selector 会返回 `CONNECTION_SELECTOR_REQUIRED`。scope 或全局 allowlist 之外的连接继续返回 `CONNECTION_OUT_OF_SCOPE`。
