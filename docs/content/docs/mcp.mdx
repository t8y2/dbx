...
DBX MCP currently provides 25 tools:
...
| `dbx_vector_delete_by_batch` | Delete one exact unpublished semantic batch with full-access permission |

Connection-scoped sessions hide connection-mutating and desktop UI tools.

## Local file imports and Milvus tools

The import tools are available only in local DBX Desktop/MCP mode. In Web mode they return the stable `IMPORT_UNSUPPORTED_IN_WEB_MODE_V1` code. Configure `DBX_MCP_IMPORT_ROOTS` with one or more real directories using the platform path separator. Every file path must be absolute, resolve inside one configured root, be a regular non-symlink file, use an allowed extension, and stay below the configured size limit. Preview supports Excel, JSON, CSV, and TSV. Governed v1 database import uses bounded streaming for XLSX/XLSM and UTF-8 CSV/TSV. Legacy `.xls`, JSON, and non-UTF-8 delimited files return stable unsupported errors before a plan or database write exists.

`dbx_prepare_table_import` binds the connection, caller-approved template version, canonical path, size, modification time, SHA-256, parsing options, and mappings into an immutable digest. Preview exposes each source column as 1-based `source_position`, exact `raw_source_name`, stable `canonical_source_name` (`name__1`, `name__2` for duplicates), and the legacy DBX name. Each mapping must repeat the position plus raw/canonical names and supply `target_column`. The server—not the caller—generates a unique `staging.mcp_<uuid>` relation, forces a new table, and makes every business column `TEXT`. The returned plan expires after 30 minutes and can be started once. Start rechecks the source and streams it into a bounded governed snapshot before opening a database pool. Every row gains `import_id`, `plan_id`, `source_sha`, the real absolute `source_row_number`, `source_row_hash`, and `loaded_at`; if exact lineage cannot be produced, the import fails before database access. Status returns the generated relation. Publication to business schemas remains a separately governed workflow.

Milvus tools accept only collections, filter fields, and output fields on their corresponding allowlists. Search requires `active_at`, always adds `approval_status == "approved"` plus effective-date filters, and optionally applies an exact `semantic_version`. Upsert accepts a strict semantic-card schema, verifies the 64-hex `content_checksum` against `content`, requires `embedding_revision`, rejects `chunk_index` and `embedding_dimension`, enforces unique card IDs and one semantic version per file, and preflights existing IDs so another batch/version cannot be overwritten. Delete requires Full access and an explicit `published: false` attestation. All entity search/query/mutation REST submitted through generic `dbx_execute_query` returns `VECTOR_DEDICATED_TOOL_REQUIRED`; only explicit management diagnostics remain available there.
