...
Connection resolution always starts from the intersection of the runtime scope and global `allowed_connection_ids`. `connection_id` resolves an exact ID. `connection_name` may resolve an exact ID or a case-insensitive name across all effective candidates. An omitted selector is accepted only when that intersection contains exactly one connection; multiple effective `DBX_MCP_SCOPE_CONNECTION_IDS` return `CONNECTION_SELECTOR_REQUIRED`. Connections outside either boundary continue to return `CONNECTION_OUT_OF_SCOPE`.
