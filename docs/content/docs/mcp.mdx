...
DBX MCP tools depend on build features, runtime scope, and the tool allowlist; the main capabilities are listed below. Daily mode excludes connection management; an explicit unscoped maintenance process adds the three connection-management tools:
...
Connection resolution always starts from the intersection of the runtime scope and the global connection allowlist, including explicit `allowed_connection_ids` and current descendants of `allowed_group_ids`. `connection_id` resolves an exact ID. `connection_name` may resolve an exact ID or a case-insensitive name across all effective candidates. An omitted selector is accepted only when connection scope is enabled and its intersection contains exactly one connection; multiple effective `DBX_MCP_SCOPE_CONNECTION_IDS` return `CONNECTION_SELECTOR_REQUIRED`. Connections outside either boundary continue to return `CONNECTION_OUT_OF_SCOPE`.
...
- `dbx_execute_batch` is not available for Redis, MongoDB, or vector database connections.

## Local file imports and Milvus tools

All eight tools recheck the tool allowlist before doing any work. Import start and vector upsert use the current effective group, connection, and database execution policies, while connection read-only and production protection remain hard limits. Prepare/start both enforce the target schema scope; start also rechecks the planned database. Status and cancellation revalidate the job's original connection, database, and schema against current scope, so old plan or job IDs do not preserve revoked access. `dbx_execute_batch` rejects vector connections so it cannot bypass the collection and semantic constraints of the dedicated vector tools.

The five import tools and the local-JSONL `dbx_vector_upsert_file` tool are available only in local DBX Desktop/MCP mode. In Web mode they return the stable `IMPORT_UNSUPPORTED_IN_WEB_MODE_V1` code. Configure `DBX_MCP_IMPORT_ROOTS` with one or more real directories using the platform path separator. Every file path must be absolute, resolve inside one configured root, be a regular non-symlink file, use an allowed extension, and stay below the configured size limit. Preview supports Excel, JSON, CSV, and TSV and explicitly reports `usedRangeExact` and `totalRowsExact`; a bounded scan is never presented as a complete row count. Governed v1 database import uses bounded streaming for XLSX/XLSM and CSV/TSV whose prepare request explicitly selects `utf-8`. Legacy `.xls`, JSON, `auto`, GBK, and UTF-16 delimited files return stable unsupported errors before a plan or database write exists.
...
```jsonc
{
  ...
  "embedding": [/* exactly the configured dimension of finite values; default 1024 */],
  "top_k": 12,
...
Both `semantic_version` and `active_at` are required; effective dates alone must never select a version. The caller should first obtain active `dataset_id + semantic_version` pairs from the published freshness catalog, then search each pair. `active_at` accepts only an `Asia/Shanghai` business date in `YYYY-MM-DD`, not a timezone-bearing datetime. `embedding` must contain exactly the finite-value count configured by `DBX_MCP_VECTOR_DIMENSION` (default 1024); the abbreviated comment above is documentation only, not valid tool input.
