...
The import tools are available only in local DBX Desktop/MCP mode. In Web mode they return the stable `IMPORT_UNSUPPORTED_IN_WEB_MODE_V1` code. Configure `DBX_MCP_IMPORT_ROOTS` with one or more real directories using the platform path separator. Every file path must be absolute, resolve inside one configured root, be a regular non-symlink file, use an allowed extension, and stay below the configured size limit. Preview supports Excel, JSON, CSV, and TSV; governed database import in v1 is deliberately limited to streaming UTF-8 CSV/TSV. Excel/JSON or non-UTF-8 prepare calls fail with a stable unsupported code before a plan or database write exists.

`dbx_prepare_table_import` binds the connection, caller-approved template version, canonical path, size, modification time, SHA-256, parsing options, and mappings into an immutable digest. Each mapping supplies the 1-based `source_position`, the exact `source_name` returned by preview, and `target_column`. The server—not the caller—generates a unique `staging.mcp_<uuid>` relation, forces a new table, and makes every business column `TEXT`. The returned plan expires after 30 minutes and can be started once. Start rechecks the source and streams it into a bounded governed snapshot before opening a database pool. Every row gains `import_id`, `plan_id`, `source_sha`, the real `source_row_number`, `source_row_hash`, and `loaded_at`; if exact lineage cannot be produced, the import fails before database access. Status returns the generated relation. Publication to business schemas remains a separately governed workflow.
