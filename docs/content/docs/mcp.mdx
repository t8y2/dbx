...
The import tools are available only in local DBX Desktop/MCP mode. In Web mode they return the stable `IMPORT_UNSUPPORTED_IN_WEB_MODE_V1` code. Configure `DBX_MCP_IMPORT_ROOTS` with one or more directories using the platform path separator. Every file path must be absolute, resolve inside one configured root, be a regular non-symlink file, use an allowed extension, and stay below the configured size limit.

`dbx_prepare_table_import` binds the connection, approved template version, canonical path, size, modification time, SHA-256, parsing options, mappings, and staging target into an immutable digest. The returned plan expires after 30 minutes and can be started once. `dbx_start_table_import` rechecks the complete file identity before launching a background import. Import targets are limited to PostgreSQL schemas listed in `DBX_MCP_IMPORT_STAGING_SCHEMAS` (default `staging`); publication to business schemas remains the caller's separately governed workflow.

Milvus tools accept only collections, filter fields, and output fields on their corresponding allowlists. Search requires `active_at`, always adds `approval_status == "approved"` plus `effective_from`/`effective_to` activity filters, and optionally applies an exact `semantic_version`. Upsert reads bounded `.jsonl` files and requires Data read/write permission. Delete accepts only an exact `semantic_batch_id` and requires Full access. Generic vector REST passed to `dbx_execute_query` is also classified by endpoint, so write or destructive POST requests cannot bypass MCP policy.
...
```jsonc
{
  ...
  "active_at": "2026-08-25T10:00:00+08:00",
...
Omit `semantic_version` when effective dates alone select the active cards. `active_at` remains required. `embedding` must contain exactly `DBX_MCP_VECTOR_DIMENSION` finite values; the abbreviated comment above is documentation only, not valid tool input.
