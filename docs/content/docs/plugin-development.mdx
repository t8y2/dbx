...
| `capabilities` | `{ downloadFile, planApi, schemaMetadataApi, dataApi, storage, ai, clipboardWrite, clipboardRead }` from the init message. A missing or `false` entry means that Host API group is unavailable here, so gate the matching call on it instead of probing with a request. |
...
- **Opt-in.** Nothing is exposed until the user enables the plugin under Plugin Center → Installed → Built-in AI tools; the same panel lists the tools and marks which ones need approval.
