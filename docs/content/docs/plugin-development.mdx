...
- `host.ai` enables the DBX-configured AI conversation API (`ai.capabilities` / `ai.complete` / `ai.stream` in the [Frontend Host API](#frontend-host-api)).
...
| `ai.capabilities()` | Reports whether DBX has a usable configured AI model; requires `host.ai`. |
| `ai.complete(request)` | Generates one reply through DBX's active AI configuration. Only messages, system prompt, and output limit cross the boundary; API keys stay in the host; requires `host.ai`. |
| `ai.stream(request)` | Streams a reply and returns `{ stream, metadata }`; stream items contain `delta`, `reasoningDelta`, and `done`. Cancelling the stream asks the host to stop generation; requires `host.ai`. |
