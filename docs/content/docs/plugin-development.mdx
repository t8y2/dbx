...
- `host.workbench` opens another workbench, `host.events` receives host events, `host.binary` enables binary channels, `host.filesystem` enables filesystem entrypoints, and `host.ai` enables the DBX-configured AI conversation API.
