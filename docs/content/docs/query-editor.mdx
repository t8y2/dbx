...
For an `UPDATE`, `INSERT`, or `DELETE` statement, the toolbar shows a **Preview changes** button (also available in the editor context menu). DBX rewrites the statement into an equivalent read-only `SELECT` and opens it in a new result tab *without writing to the database*:

- `UPDATE` — the grid shows the affected rows plus one **`列名（新值）`** column per assigned column (expressions are evaluated, so `SET a = a + 1` shows the actual new value);
...
Statements DBX cannot rewrite safely (for example some dialect-specific forms) are reported as **暂不支持预览** and can still be executed normally.
