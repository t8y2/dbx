...
Use `${1}`, `${2}`, etc. in templates to mark editable positions. Add default text after a colon to make the intended object clear, for example:
...
After expansion, press `Tab` to jump between placeholders. Placeholders with the same number are edited simultaneously.
