...
<Tabs groupId="ssh-auth" items={['Private Key (Recommended)', 'Private Key + Password', 'Password']}>
  ...
  <Tab value="Private Key + Password">
    Use a private key as the primary credential with password as a fallback. DBX tries key authentication first; if the server rejects the key, it automatically falls back to password authentication.
