...
The first Test or Connect that would upload the worker asks for consent and shows the destination path and digest. You can keep the worker for the session only, persist it under `~/.cache/dbx/sqlite-worker/<digest>`, or point at a pre-placed binary that matches this DBX build. SQLCipher, loadable extensions, and attached databases are not supported on that remote path in v1. Backup and restore write or read files on the file host.
