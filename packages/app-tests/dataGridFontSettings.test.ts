...
test("invalidates grid measurements and canvas rendering when the result font changes", () => {
  assert.match(dataGridSource, /columnHeaderMeasurementKey = computed\(\(\) => \[tableFontSize\.value, tableFontFamily\.value, dataGridFontReadyTick\.value, columnFormatterReadyTick\.value\]\)/);
