...
test("provides official Phoenix JDBC defaults for both connection modes", () => {
  ...
  assert.deepEqual(phoenixConnectionDefaults("direct"), {
    connectionString: PHOENIX_DIRECT_JDBC_URL,
    driverClass: PHOENIX_DIRECT_JDBC_DRIVER_CLASS,
  });
  assert.deepEqual(phoenixConnectionDefaults("query-server"), {
...
test("hydrates only the detected mode and preserves custom Phoenix fields", () => {
  const fields = createPhoenixConnectionFieldsByMode({
  ...
  assert.deepEqual(fields.direct, phoenixConnectionDefaults("direct"));
...
test("remembers independent custom values while switching modes", () => {
  let fields = createPhoenixConnectionFieldsByMode();
  fields = rememberPhoenixConnectionFields(fields, "direct", {
    connectionString: "jdbc:phoenix:zk1,zk2:2181:/hbase-secure",
    driverClass: "com.example.CustomPhoenixDriver",
  });
  fields = rememberPhoenixConnectionFields(fields, "query-server", {
...
test("recognizes only generated Phoenix driver classes as replaceable defaults", () => {
  assert.equal(isPhoenixDefaultDriverClass(undefined), true);
  assert.equal(isPhoenixDefaultDriverClass(PHOENIX_DIRECT_JDBC_DRIVER_CLASS), true);
  assert.equal(isPhoenixDefaultDriverClass(PHOENIX_QUERY_SERVER_JDBC_DRIVER_CLASS), true);
  assert.equal(isPhoenixDefaultDriverClass("com.example.CustomPhoenixDriver"), false);
});

test("defines mode-specific Phoenix Maven runtimes", () => {
  assert.deepEqual(phoenixMavenCoordinates("direct"), [PHOENIX_DIRECT_MAVEN_COORDINATE, PHOENIX_DIRECT_LOGGING_MAVEN_COORDINATE]);
  assert.deepEqual(phoenixMavenCoordinates("query-server"), [PHOENIX_QUERY_SERVER_MAVEN_COORDINATE]);
  ...
  assert.equal(isPhoenixManagedMavenPath("/plugins/jdbc/drivers/maven/org.apache.phoenix_phoenix-queryserver-client_6.0.0/jars/phoenix-queryserver-client-6.0.0.jar"), true);
  assert.equal(isPhoenixManagedMavenPath("/opt/phoenix-queryserver-client-6.0.0.jar"), false);
  assert.equal(isPhoenixManagedMavenCoordinate(PHOENIX_DIRECT_MAVEN_COORDINATE), true);
  assert.equal(isPhoenixManagedMavenCoordinate("com.example:custom-driver:1.0.0"), false);
  assert.equal(phoenixRuntimeSelectionId("direct"), "phoenix:direct");
});
...
test("builds one complete selector runtime for each installed Phoenix mode", () => {
  ...
  assert.deepEqual(phoenixManagedRuntimePaths([direct, logging, queryServer], "direct"), [direct.artifacts[0].path, logging.artifacts[0].path]);
  assert.deepEqual(phoenixManagedRuntimePaths([direct, logging, queryServer], "query-server"), [queryServer.artifacts[0].path]);
  assert.deepEqual(phoenixManagedRuntimePaths([direct], "direct"), []);
});
...
test("requires the JDBC plugin to be installed from Driver Manager", async () => {
  ...
  await assert.rejects(ensurePhoenixRuntimeDrivers(config, runtime.api), new RegExp(PHOENIX_JDBC_PLUGIN_NOT_INSTALLED_ERROR.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.equal(runtime.listJdbcMavenBundles.mock.calls.length, 0);
  assert.equal(isPhoenixDriverInstallError(PHOENIX_JDBC_PLUGIN_NOT_INSTALLED_ERROR), true);
});
...
test("does not download a missing Direct runtime while testing or connecting", async () => {
  ...
  await assert.rejects(ensurePhoenixRuntimeDrivers(config, runtime.api), new RegExp(PHOENIX_DRIVER_NOT_INSTALLED_ERROR.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.equal(runtime.listJdbcMavenBundles.mock.calls.length, 1);
  assert.equal(isPhoenixDriverInstallError(PHOENIX_DRIVER_NOT_INSTALLED_ERROR), true);
});
...
test("reuses installed Direct bundles, removes stale managed PQS paths, and preserves configuration paths", async () => {
  ...
  const result = await ensurePhoenixRuntimeDrivers(config, runtime.api);
...
test("does not provision Phoenix runtimes for another JDBC profile", async () => {
  ...
  assert.equal(await ensurePhoenixRuntimeDrivers(config, runtime.api), undefined);
  assert.equal(runtime.jdbcPluginStatus.mock.calls.length, 0);
  assert.equal(runtime.listJdbcMavenBundles.mock.calls.length, 0);
});

test("registers Apache Phoenix as a first-class JDBC picker profile with both modes", () => {
  const source = readFileSync(path.resolve("apps/desktop/src/components/connection/ConnectionDialog.vue"), "utf8");

  assert.match(source, /phoenix:\s*\{\s*type:\s*"jdbc",\s*port:\s*0,\s*user:\s*"",\s*label:\s*"Apache Phoenix",\s*icon:\s*"phoenix"\s*\}/);
  assert.match(source, /\{\s*value:\s*"phoenix",\s*label:\s*"Apache Phoenix"\s*\}/);
  assert.match(source, /applyPhoenixConnectionMode\('direct'\)/);
  assert.match(source, /applyPhoenixConnectionMode\('query-server'\)/);
});
