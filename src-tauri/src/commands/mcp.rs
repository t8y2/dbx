...
impl NodeRuntime {
    fn probe(candidate: NodeRuntimeCandidate) -> Option<Self> {
        ...
        let package_is_compatible = match package.as_ref().and_then(|package| package.minimum_node_version) {
            Some(minimum) => parse_node_version(&node_version).is_some_and(|version| version >= minimum),
            None => true,
        };
